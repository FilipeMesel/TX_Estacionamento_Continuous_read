...
int FXOS8700CQCalibrate(uint8_t type)
{
	...
	for(int i =0; i < 10; i++)
	{
		...
		{
			int auxiliar = (int)(FXOS8700CQGetData(GET_MAG_X));
			if(auxiliar == 0)
			{
				i--;
				if(i < 0)
...
static void OnTimer(void *contextid){
	...
	if(ind == 0){
		...
		FXOS8700CQReadAccelMagnData(&hi2c1);
		int16_t magX = FXOS8700CQGetData(GET_MAG_X);
		int16_t magY = FXOS8700CQGetData(GET_MAG_Y);
		int16_t magZ = FXOS8700CQGetData(GET_MAG_Z);
...
uint16_t fillBufferAutoTx(uint8_t *buffer){
	...
	int16_t tmp = FXOS8700CQGetData(GET_MAG_X);
